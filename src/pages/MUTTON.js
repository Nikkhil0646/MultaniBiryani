import styles from "./MUTTON.module.css";

const MUTTON = () => {
  return (
    <div className={styles.ourMenu}>
      <div className={styles.main}>
        <a href="/">
          <img
            className={styles.contentIcon}
            loading="lazy"
            alt=""
            src="/arrow-down--undefined--glyph-undefined@2x.png"
          />
        </a>
      </div>
      <main className={styles.navigation}>
        <div className={styles.menuTitle}>
          <div className={styles.ourMenu1}>Mutton</div>
        </div>
        <div className={styles.grid}>
          <div className={styles.gridItemContainer}>
          <div className={styles.gridItem}>
              <img
                className={styles.images}
                alt=""
                src="/fd5d9d23c721867b8c05562261dcb2d51.png"
              />
            </div>
            <div className={styles.biryaniWrapper}>
              <div className={styles.biryani}>Mutton Stew</div>
              <div className={styles.priceHeaderRow}>
                <div className={styles.priceNameRow}>
                  <a className={styles.prices}>Prices :</a>
                </div>
                <div className={styles.frameParent}>
                  <div className={styles.frameWrapper}>
                    <div className={styles.kgParent}>
                      <div className={styles.kg3}>1/4 kg</div>
                      <div className={styles.kg}>1/2 kg</div>
                      <div className={styles.kg1}>1 kg</div>
                    </div>
                  </div>
                  <div className={styles.parent}>
                    <div className={styles.div}>₹350/-</div>
                    
                    <div className={styles.div1}>₹700/-</div>
                    <div className={styles.wrapper}>
                      <div className={styles.div1}>₹1400/-</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles.gridItemContainer}>
          <div className={styles.gridItem}>
              <img
                className={styles.images}
                alt=""
                src="/eb4593a9c14f857e0b93673a7f2e2ef01.png"
              />
            </div>
            <div className={styles.riceWrapper}>
              <div className={styles.rice}>Mutton Keema Kalegi</div>
              <div className={styles.priceHeaderRow}>
                <div className={styles.priceNameRow}>
                  <a className={styles.prices}>Prices :</a>
                </div>
                <div className={styles.frameParent}>
                  <div className={styles.frameWrapper}>
                    <div className={styles.kgParent}>
                      <div className={styles.kg3}>1/4 kg</div>
                      <div className={styles.kg}>1/2 kg</div>
                      <div className={styles.kg1}>1 kg</div>
                    </div>
                  </div>
                  <div className={styles.parent}>
                    <div className={styles.div}>₹350/-</div>
                    
                    <div className={styles.div1}>₹350/-</div>
                    <div className={styles.wrapper}>
                      <div className={styles.div1}>₹700/-</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles.gridItemContainer}>
          <div className={styles.gridItem}>
              <img
                className={styles.images}
                alt=""
                src="/9937caeb49292fc51075d8adc6281d921.png"
              />
            </div>
            <div className={styles.muttonWrapper}>
              <div className={styles.mutton}>Mutton Rezala</div>
              <div className={styles.priceHeaderRow}>
                <div className={styles.priceNameRow}>
                  <a className={styles.prices}>Prices :</a>
                </div>
                <div className={styles.frameParent}>
                  <div className={styles.frameWrapper}>
                    <div className={styles.kgParent}>
                      <div className={styles.kg3}>1/4 kg</div>
                      <div className={styles.kg}>1/2 kg</div>
                      <div className={styles.kg1}>1 kg</div>
                    </div>
                  </div>
                  <div className={styles.parent}>
                    <div className={styles.div}>₹350/-</div>
                    
                    <div className={styles.div1}>₹700/-</div>
                    <div className={styles.wrapper}>
                      <div className={styles.div1}>₹1400/-</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles.gridItemContainer}>
          <div className={styles.gridItem}>
              <img
                className={styles.images}
                alt=""
                src="/25a1338323358fc94e04134f55df5cb61.png"
              />
            </div>
            <div className={styles.chickenWrapper}>
              <div className={styles.chicken}>Mutton Rogan Josh</div>
              <div className={styles.priceHeaderRow}>
                <div className={styles.priceNameRow}>
                  <a className={styles.prices}>Prices :</a>
                </div>
                <div className={styles.frameParent}>
                  <div className={styles.frameWrapper}>
                    <div className={styles.kgParent}>
                      <div className={styles.kg3}>1/4 kg</div>
                      <div className={styles.kg}>1/2 kg</div>
                      <div className={styles.kg1}>1 kg</div>
                    </div>
                  </div>
                  <div className={styles.parent}>
                    <div className={styles.div}>₹350/-</div>
                    
                    <div className={styles.div1}>₹700/-</div>
                    <div className={styles.wrapper}>
                      <div className={styles.div1}>₹1400/-</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
};

export default MUTTON;